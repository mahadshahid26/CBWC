namespace CBWC.Application.Common.ResultWrapper.Extensions;
public static class ResponseExtensions
{
    public static T? Conversion<T>(this object? model)
    {
        if (model is T typedPayload)
        {
            return typedPayload;
        }
        ...
        {
            return ShouldConvertToArray(sourceType, typeof(T)) ? ConvertToArray<T>(model) : ShouldConvertToList(sourceType) ? ConvertToList<T>(model) : default!;
        }
        else
        {
            return ConvertBasedOnSimilarProperties<T>(model);
        }
    }
